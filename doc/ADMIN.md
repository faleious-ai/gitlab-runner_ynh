...
The current YunoHost config-panel action `main.registration.register` uses the stored URL, token and Docker image settings. Values are validated as a set before registration; the token is passed to GitLab Runner through its documented environment variables and never through `argv`. Diagnostics redact the token. The test fixture uses only a non-functional placeholder. Do not paste a production token into repository files or issue reports.
